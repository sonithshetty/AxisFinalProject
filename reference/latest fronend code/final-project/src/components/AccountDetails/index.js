...
class AccountDetails extends Component {
  ...
  handleCheckboxChange = (event, index, id) => {
    ...
    if (name === "modify") {
      const rowToSelect = faxList.find((eachData) => eachData.id === id && checked);
      if (rowToSelect) {
        this.setState({ toUpdateList: [...this.state.toUpdateList, rowToSelect] });
      }
    ...
    if (name === "verify") {
      ...
      if (checked) {
        ...
        const updateCancelListIds = cancelListIds.filter(
          (each) => each !== id
        );
