...
function Table() {
    const [data, setData] = useState([
        { name: '', email: '', faxNumber: 0, refNumber: '', modify: false, verify: false, digitalSignature: false, delete: false },
        { name: '', email: '', faxNumber: 0, refNumber: '', modify: false, verify: false, digitalSignature: false, delete: false },
        { name: '', email: '', faxNumber: 0, refNumber: '', modify: false, verify: false, digitalSignature: false, delete: false },
        { name: '', email: '', faxNumber: 0, refNumber: '', modify: false, verify: false, digitalSignature: false, delete: false },
        { name: '', email: '', faxNumber: 0, refNumber: '', modify: false, verify: false, digitalSignature: false, delete: false },
    ...
    const submitData = async event => {
        ...
        const modifiedData = data.filter(row => row.name !== "" || row.email !== "" || row.faxNumber !== 0 || row.refNumber !== "")
                                 .map(row => ({ 
                                            name: row.name,
                                            emailId: row.email,
        ...
        <form onSubmit={submitData}>
            <table>
                ...
                <tbody>
                    {data.map((row, index) => (
                        <tr key={index}>
                            <td><input type="text" name="name" value={row.name} onChange={(event) => handleInputChange(event, index)} /></td>
                            <td><input type="email" name="email" value={row.email} onChange={(event) => handleInputChange(event, index)} /></td>
                            <td><input type="number" name="faxNumber" value={row.faxNumber} onChange={(event) => handleInputChange(event, index)} /></td>
                            <td><input type="text" name="refNumber" disabled={!isDigitalSignatureChecked || !row.digitalSignature} value={row.refNumber} onChange={(event) => handleInputChange(event, index)} /></td>
                            <td><input type="checkbox" name="digitalSignature" checked={row.digitalSignature} onChange={(event) => handleCheckboxChange(event, index)} /></td>
                            <td><input type="checkbox" name="delete" disabled={!isDeleteChecked} checked={row.delete} onChange={(event) => handleCheckboxChange(event, index)} /></td>
                            <td><input type="checkbox" name="modify" disabled={!isModifyChecked} checked={row.modify} onChange={(event) => handleCheckboxChange(event, index)} /></td>
                            <td><input type="checkbox" name="verify" disabled={!isVerifyChecked} checked={row.verify} onChange={(event) => handleCheckboxChange(event, index)} /></td>
                        </tr>
                    ))} 
                </tbody>
            </table>
